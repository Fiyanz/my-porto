...
export default function ContactClient({ profile }: { profile: any }) {
  const getUsername = (url: string) => {
    if (!url) return '';
    return url.replace(/\/$/, '').replace(/\?.*$/, '').split('/').pop();
  };

  const [purpose, setPurpose] = useState('Just Saying Hi');
    ...
    <>
      <div className="flex items-end justify-between mb-5">
        ...
        <div className="flex items-center gap-2 border-2 border-black bg-white px-4 py-2">
          ...
          <span className="text-xs font-black">{profile?.status || "Available for Internship"}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="col-span-1 md:col-span-2 flex flex-col gap-4">
          <div id="channels-row" className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <a href={profile?.github_url || "https://github.com"} target="_blank" className="border-2 border-black bg-white p-4 flex items-center gap-3 group cursor-pointer hover:bg-gray-50">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm font-black truncate">{profile?.github_url ? `@${getUsername(profile.github_url)}` : '@username'}</div>
                <div className="text-xs text-gray-400 mono mt-0.5">Primary code repository</div>
              </div>
              <i className="fa-solid fa-arrow-up-right-from-square text-xs text-gray-400"></i>
            </a>

            <a href={profile?.linkedin_url || "https://linkedin.com"} target="_blank" className="border-2 border-black bg-white p-4 flex items-center gap-3 cursor-pointer hover:bg-gray-50">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm font-black truncate">{profile?.name || 'Your Name'}</div>
                <div className="text-xs text-gray-400 mono mt-0.5">Fastest response</div>
              </div>
              <i className="fa-solid fa-arrow-up-right-from-square text-xs text-gray-400"></i>
            </a>

            <a href={`mailto:${profile?.email || 'email@example.com'}`} className="border-2 border-black bg-white p-4 flex items-center gap-3 cursor-pointer hover:bg-gray-50">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm font-black truncate">{profile?.email || 'email@example.com'}</div>
            ...
            {profile?.instagram_url ? (
              <a href={profile.instagram_url} target="_blank" className="border-2 border-black bg-white p-4 flex items-center gap-3 cursor-pointer hover:bg-gray-50">
                <div className="w-10 h-10 bg-gray-300 border-2 border-black flex items-center justify-center shrink-0">
                  <i className="fa-brands fa-instagram text-gray-900 text-lg"></i>
                </div>
                <div className="flex-1">
                  <div className="text-xs text-gray-500 font-medium">Instagram</div>
                  <div className="text-sm font-black truncate">@{getUsername(profile.instagram_url)}</div>
                  <div className="text-xs text-gray-400 mono mt-0.5">Casual updates</div>
                </div>
                <i className="fa-solid fa-arrow-up-right-from-square text-xs text-gray-400"></i>
              </a>
            ) : (
              <a href="#" className="border-2 border-black bg-white p-4 flex items-center gap-3 cursor-pointer hover:bg-gray-50">
                <div className="w-10 h-10 bg-gray-300 border-2 border-black flex items-center justify-center shrink-0">
                  <i className="fa-brands fa-instagram text-gray-900 text-lg"></i>
                </div>
                <div className="flex-1">
          ...
          <div id="contact-form" className="border-2 border-black bg-white p-5">
            ...
            <div className="mb-3">
              ...
              <div className="flex border-2 border-black overflow-hidden text-xs font-bold">
                {['Just Saying Hi', 'Internship', 'Collaboration', 'Other'].map((p, idx) => (
                  <button
                    key={p}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div id="quick-links" className="border-2 border-black bg-white p-4">
            ...
            <div className="flex flex-col gap-2">
              <a href={profile?.cv_url || "/api/files/cv"} target="_blank" className="flex items-center justify-between border-2 border-black px-3 py-2 bg-gray-50 hover:bg-gray-100 cursor-pointer">
                <span className="text-xs font-bold">Download CV</span>
                <i className="fa-solid fa-download text-xs"></i>
              </a>
              <a href={profile?.github_url || "https://github.com/Fiyanz"} target="_blank" className="flex items-center justify-between border-2 border-black px-3 py-2 bg-gray-50 hover:bg-gray-100 cursor-pointer">
                <span className="text-xs font-bold">GitHub Profile</span>
                <i className="fa-solid fa-arrow-up-right-from-square text-xs"></i>
              </a>
              <Link href="/projects" className="flex items-center justify-between border-2 border-black px-3 py-2 bg-gray-50 hover:bg-gray-100 cursor-pointer">
                <span className="text-xs font-bold">View Projects</span>
                <i className="fa-solid fa-folder-open text-xs"></i>
              </Link>
              {profile?.instagram_url ? (
                <a href={profile.instagram_url} target="_blank" className="flex items-center justify-between border-2 border-black px-3 py-2 bg-gray-50 hover:bg-gray-100 cursor-pointer">
                  <span className="text-xs font-bold">Instagram</span>
                  <i className="fa-brands fa-instagram text-xs"></i>
                </a>
              ) : (
                <a href="#" className="flex items-center justify-between border-2 border-black px-3 py-2 bg-gray-50 hover:bg-gray-100 cursor-pointer">
                  <span className="text-xs font-bold">Instagram</span>
                  <i className="fa-brands fa-instagram text-xs"></i>
                </a>
              )}
          ...
          <div id="terminal-hint" className="p-4 rounded-xl shadow-lg" style={{ backgroundColor: '#1a1b26', border: '2px solid #3b3d57' }}>
            <div className="mono text-xs mb-2">
              <span style={{ color: '#7aa2f7' }}>{profile?.email ? profile.email.split('@')[0] : 'bagus'}@hyprland-rice</span>
              <span style={{ color: '#a9b1d6' }}> ~ </span>
              <span style={{ color: '#9ece6a' }}>$</span>
            </div>
            <div className="mono text-xs leading-relaxed" style={{ color: '#a9b1d6' }}>
              Or just open the Kitty terminal below and type <span style={{ color: '#9ece6a', fontWeight: 'bold' }}>'sudo hire'</span> — it's faster 😄
            </div>
          </div>
        </div>
      </div>
